        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            else
            {
                ...
                if (!(wtx.IsCoinBase() || wtx.IsCoinStake()))
                    wtx.AcceptWalletTransaction(state, txdb, false);
            }
        }
        if (!vMissingTx.empty())
        {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        if (fKernelFound || fShutdown || (fCreateCoinStakeSleep && fGoSleep))
...
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, int &nNumberOfThisError, std::string &strError)
{
    ...
    if (block.CheckFork(uianLastHashCheckPointPrev, uianLastHashCheckPoint, uiquNewBlockHash, false, true) && !fIgnoreForkWhenSending)
    {
    ...
    {
        LOCK2(cs_main, cs_wallet);
        printf("'CWallet->CommitTransaction' : \n%s", wtxNew.ToString().c_str());
        {
        ...
        if (!wtxNew.GoMerkleTxToMemoryPool(txdb, true, false))
        {
            ...
            printf("'CWallet->CommitTransaction' : Error: Transaction not valid\n");
            return false;
        }
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s CACHE %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s CACHE %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
