...
bool CBlock::SetVirtualCheckPointHashes(uint256 uiHashLeft, uint256 uiHashRight)
{
                ...
                if (newblockindex->pprev->GetBlockHash() == bestblockindex->pprev->GetBlockHash())
                {
                    ...
                    if (newblockindex->GetBlockTime() > bestblockindex->GetBlockTime() || fLesserHash)
                    {
                        ...
                        if (!bimapVirtualCheckPointBlockIndex.right.count(Checkpoints::hashSyncCheckpoint) && fResetSyncCheckpoint)
                        {
                            ...
                            SetVirtualCheckPointHashes(bestblockindex->pprev->GetBlockHash(), Checkpoints::hashSyncCheckpoint);
                        }
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf(" 'GenerateBitcoins()' - %d processors\n", nProcessors);
        if (nProcessors < 1)
            nProcessors = 1;
        if (fLimitProcessors && nProcessors > nLimitProcessors)
            nProcessors = nLimitProcessors;
        int nAddThreads = nProcessors - vnThreadsRunning[THREAD_MINER];
        printf(" 'GenerateBitcoins()' - Starting %d BitcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinMiner, pwallet))
                printf(" 'GenerateBitcoins()' - Error: NewThread(ThreadBitcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
