...
//    dbenv.log_set_config(DB_LOG_AUTO_REMOVE, 1);
    ...
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a single block. If this limit is reached, 'CACHE'Project may stop working."), (unsigned long)nMaxLocks);
            else
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a common blockchain reorganization. If this limit is reached, 'CACHE'Project may stop working."), (unsigned long)nMaxLocks);
...
bool CTxDB::LoadBlockIndex()
{
    ...
    int nCheckDepth = GetArg( "-checkblocks", 160); // down from 2500 to speed up start, but should be >520
    if (nCheckDepth == 0)
    ...
    for (CBlockIndex* pindex = pindexBest; pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel>0 && !block.CheckBlock(state, false))
        {
